import { FACEBOOK_USERNAME, GITHUB_USERNAME, LINKEDIN_USERNAME, TWITTER_USERNAME } from '@/config';
...
const SocialIcons: React.FC = () => {
	const socialLinks = [
		...
		{ icon: 'github',href: `https://www.github.com/${GITHUB_USERNAME}/short4u` },
	];
	return (
		<>
			<div className="flex -ml-2 flex-col space-x-3 mt-2 border-t p-2">
				...
				<div className='flex  gap-x-2 justify-start items-center mt-2 flex-wrap'>
					{socialLinks.map((link,index) => (
						<SocialIcon key={index} icon={link.icon as SocialIconProps["icon"]} href={link.href} />
					))}
				</div>
			</div>
